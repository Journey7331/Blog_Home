...
function checkForm(){
	...
	if(userName==null||userName==""){
		$("#error").html("User can't be empty!");
		return false;
	}
	if(password==null||password==""){
		$("#error").html("Password can't be empty!");
		return false;
	}
	return true;
}
...
<form action="${pageContext.request.contextPath}/blogger/login.do" method="post" onsubmit="return checkForm()">
	<DIV style="background: rgb(255, 255, 255); margin: -100px auto auto; border: 1px solid rgb(231, 231, 231); border-image: none; width: 400px; height: 200px; text-align: center;">
		...
		<P style="padding: 30px 0px 10px; position: relative;">
			...
			<INPUT id="userName" name="userName" class="ipt" type="text" placeholder="Input Username" value="${blogger.userName }"> 
	    </P>
		<P style="position: relative;">
			<SPAN class="p_logo"></SPAN>         
			<INPUT id="password" name="password" class="ipt"  type="password" placeholder="Input Password" value="${blogger.password }">   
	  	</P>
		<DIV style="height: 50px; line-height: 50px; margin-top: 30px; border-top-color: rgb(231, 231, 231); border-top-width: 1px; border-top-style: solid;">
			<P style="margin: 0px 35px 20px 45px;">
			<SPAN style="float: left;"><a href="${pageContext.request.contextPath}/index.html">My Blog</a></SPAN> 
			<span><font color="red" id="error">${errorInfo }</font></span>
	        <SPAN style="float: right;"> 
	              <input type="submit" style="background: rgb(0, 142, 173); padding: 7px 10px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold;" value="Log in"/> 
	         </SPAN>         
	         </P>
	    </DIV>
	</DIV>
